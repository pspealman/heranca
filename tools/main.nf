 ...
 *
 *  Edited: 08.19.2022 Pieter Spealman < ps163@nyu.edu >
 *		1) Set haplotypeCaller ploidy to 1 (For use in haploid S. cerevisiae)
 *		2) Set the variantfilter to accept MQ scores above 30 (previously "> 40") - this reflects the lower bound of accuracy as validated by long read sequencing
 */
...
process gitclone_erisapfel{
	output:
		path erisapfel into gitrepo_erisapfel

  script:
	"""
		git clone https://github.com/pspealman/erisapfel.git
	"""
}

process erisapfel {
	publishDir "${params.out}/erisapfel_out", mode: 'copy'

  input:
  set pair_id,
		file(reads) from read_pairs_ch2
	path gitrepo from gitrepo_erisapfel

	output:
		file("results/${pair_id}/output/${pair_id}_realigned.gff") into jbrowse_erisapfel_out_ch
		//file("results/${pair_id}/output/${pair_id}_realigned.vcf.gz") into jbrowse_erisapfel_out_ch
	...
	#define local copy name
	fastq_1=${pair_id}_n01.fastq.gz
	fastq_2=${pair_id}_n02.fastq.gz
  ...
  cp ${reads[1]} \${fastq_2}
	...
	#run erisapfel
	python ${gitrepo}/erisapfel.py -make -fa ${params.ref} -fastq_1 \${fastq_1} -fastq_2 \${fastq_2} -run_name ${pair_id}
	python ${gitrepo}/erisapfel.py -depth -run_name ${pair_id}
	python ${gitrepo}/erisapfel.py -peaks -run_name ${pair_id}
	python ${gitrepo}/erisapfel.py -map -run_name ${pair_id}
	python ${gitrepo}/erisapfel.py -localseq -run_name ${pair_id}
	"""
...
process markDuplicatesSpark {
    ...
    set val(pair_id),
        ...
        into full_bam_bw_ch, downsample_bam_ch, qualimap_ch, bamcompare_ch
...
process bamcompare{
	publishDir "${params.out}/bamcompare", mode:'copy'
  ...
  output:
		file("${id}_log2ratio.bw") into bamcompare_out_ch
...
process jbrowse{
    ...
    file '*' from bw_out_ch.collect()
		file '*' from	jbrowse_erisapfel_out_ch.collect()
		file '*' from bamcompare_out_ch.collect()
